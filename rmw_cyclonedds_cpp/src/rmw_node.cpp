...
/* Security must be enabled when compiling and requires cyclone to support QOS
 * property lists */
...
/* Set to != 0 for periodically printing requests that have been blocked for
 * more than 1s */
...
/* instance handles are unsigned 64-bit integers carefully constructed to be as
   close to uniformly distributed as possible for no other reason than making
   them near-perfect hash keys, hence we can improve over the default hash
   function */
...
bool operator<(dds_builtintopic_guid_t const &a,
               dds_builtintopic_guid_t const &b) {
  return memcmp(&a, &b, sizeof(dds_builtintopic_guid_t)) < 0;
}

static rmw_ret_t discovery_thread_stop(rmw_dds_common::Context &context);
static bool dds_qos_to_rmw_qos(const dds_qos_t *dds_qos,
                               rmw_qos_profile_t *qos_policies);

static rmw_publisher_t *
create_publisher(dds_entity_t dds_ppant, dds_entity_t dds_pub,
...
create_subscription(dds_entity_t dds_ppant, dds_entity_t dds_pub,
                    const rosidl_message_type_support_t *type_supports,
...
struct Cdds {
  ...
  /* special guard condition that gets attached to every waitset but that is
     never triggered: this way, we can avoid Cyclone's behaviour of always
     returning immediately when no entities are attached to a waitset */
  dds_entity_t gc_for_empty_waitset;

  /* set of waitsets protected by lock, used to invalidate all waitsets caches
...
/* Use construct-on-first-use for the global state rather than a plain global
   variable to prevent its destructor from running prior to last use by some
   other component in the system.  E.g., some rclcpp tests (at the time of this
   commit) drop a guard condition in a global destructor, but (at least) on
   Windows the Cyclone RMW global dtors run before the global dtors of that
   test, resulting in rmw_destroy_guard_condition() attempting to use the
   already destroyed "Cdds::waitsets".

   The memory leak this causes is minor (an empty map of domains and an empty
   set of waitsets) and by definition only once.  The alternative of elimating
   it altogether or tying its existence to init/shutdown is problematic because
   this state is used across domains and contexts.

   The only practical alternative I see is to extend Cyclone's run-time state
   (which is managed correctly for these situations), but it is not Cyclone's
   responsibility to work around C++ global destructor limitations. */
static Cdds &gcdds() {
...
struct CddsDomain {
  /* This RMW implementation currently implements localhost-only by explicitly
     creating domains with a configuration that consists of: (1) a hard-coded
     selection of "localhost" as the network interface address; (2) followed by
     the contents of the CYCLONEDDS_URI environment variable:

     - the "localhost" hostname should resolve to 127.0.0.1 (or equivalent) for
     IPv4 and to ::1 for IPv6, so we don't have to worry about which of IPv4 or
     IPv6 is used (as would be the case with a numerical IP address), nor do we
     have to worry about the name of the loopback interface;

     - if the machine's configuration doesn't properly resolve "localhost", you
     can still override via $CYCLONEDDS_URI.

     The CddsDomain type is used to track which domains exist and how many nodes
     are in it.  Because the domain is instantiated with the first nodes created
     in that domain, the other nodes must have the same localhost-only setting.
     (It bugs out if not.) Everything resets automatically when the last node in
     the domain is deleted.

     (It might be better still to for Cyclone to provide "loopback" or something
     similar as a generic alias for a loopback interface ...)

     There are a few issues with the current support for creating domains
     explicitly in Cyclone, fixing those might relax alter or relax some of the
     above. */
  ...
  CddsDomain() : refcount(0), domain_handle(0) {
  ...
  ~rmw_context_impl_s() {
    if (0u != this->node_count) {
      RCUTILS_SAFE_FWRITE_TO_STDERR(
          ...
          "Ensure `rcl_node_fini` is called for all nodes before "
          "`rcl_context_fini`,"
          "to avoid leaking.\n");
    }
  }
...
extern "C" rmw_ret_t rmw_set_log_severity(rmw_log_severity_t severity) {
  ...
  switch (severity) {
  default:
    RMW_SET_ERROR_MSG_WITH_FORMAT_STRING("%s: Invalid log severity '%d'",
                                         __func__, severity);
...
#define MAKE_DDS_EVENT_CALLBACK_FN(event_type, EVENT_TYPE)                     \
  static void on_##event_type##_fn(dds_entity_t entity,                        \
...
static void listener_set_event_callbacks(dds_listener_t *l, void *arg) {
  dds_lset_requested_deadline_missed_arg(l, on_requested_deadline_missed_fn,
                                         arg, false);
  dds_lset_requested_incompatible_qos_arg(l, on_requested_incompatible_qos_fn,
                                          arg, false);
  dds_lset_sample_lost_arg(l, on_sample_lost_fn, arg, false);
  dds_lset_liveliness_lost_arg(l, on_liveliness_lost_fn, arg, false);
  dds_lset_offered_deadline_missed_arg(l, on_offered_deadline_missed_fn, arg,
                                       false);
  dds_lset_offered_incompatible_qos_arg(l, on_offered_incompatible_qos_fn, arg,
                                        false);
...
static bool get_readwrite_qos(dds_entity_t handle,
...
extern "C" rmw_ret_t rmw_subscription_set_on_new_message_callback(
    rmw_subscription_t *rmw_subscription, rmw_event_callback_t callback,
...
extern "C" rmw_ret_t
rmw_service_set_on_new_request_callback(rmw_service_t *rmw_service,
...
extern "C" rmw_ret_t
rmw_client_set_on_new_response_callback(rmw_client_t *rmw_client,
                                        rmw_event_callback_t callback,
                                        const void *user_data) {
...
template <typename T>
static void event_set_callback(T event, dds_status_id_t status_id,
                               rmw_event_callback_t callback,
                               const void *user_data) {
...
extern "C" rmw_ret_t rmw_event_set_callback(rmw_event_t *rmw_event,
                                            rmw_event_callback_t callback,
                                            const void *user_data) {
  ...
  case RMW_EVENT_LIVELINESS_CHANGED: {
    ...
    event_set_callback(sub_event, DDS_LIVELINESS_CHANGED_STATUS_ID, callback,
                       user_data);
    break;
  }
  ...
  case RMW_EVENT_REQUESTED_DEADLINE_MISSED: {
    auto sub_event = static_cast<CddsSubscription *>(rmw_event->data);
    event_set_callback(sub_event, DDS_REQUESTED_DEADLINE_MISSED_STATUS_ID,
                       callback, user_data);
    break;
  }
  ...
  case RMW_EVENT_REQUESTED_QOS_INCOMPATIBLE: {
    auto sub_event = static_cast<CddsSubscription *>(rmw_event->data);
    event_set_callback(sub_event, DDS_REQUESTED_INCOMPATIBLE_QOS_STATUS_ID,
                       callback, user_data);
    break;
  }
  ...
  case RMW_EVENT_MESSAGE_LOST: {
    auto sub_event = static_cast<CddsSubscription *>(rmw_event->data);
    event_set_callback(sub_event, DDS_SAMPLE_LOST_STATUS_ID, callback,
                       user_data);
    break;
  }
  ...
  case RMW_EVENT_SUBSCRIPTION_MATCHED: {
    auto sub_event = static_cast<CddsSubscription *>(rmw_event->data);
    event_set_callback(sub_event, DDS_SUBSCRIPTION_MATCHED_STATUS_ID, callback,
                       user_data);
    break;
  }
  ...
  case RMW_EVENT_LIVELINESS_LOST: {
    ...
    event_set_callback(pub_event, DDS_LIVELINESS_LOST_STATUS_ID, callback,
                       user_data);
    break;
  }
  ...
  case RMW_EVENT_OFFERED_DEADLINE_MISSED: {
    auto pub_event = static_cast<CddsPublisher *>(rmw_event->data);
    event_set_callback(pub_event, DDS_OFFERED_DEADLINE_MISSED_STATUS_ID,
                       callback, user_data);
    break;
  }
  ...
  case RMW_EVENT_OFFERED_QOS_INCOMPATIBLE: {
    auto pub_event = static_cast<CddsPublisher *>(rmw_event->data);
    event_set_callback(pub_event, DDS_OFFERED_INCOMPATIBLE_QOS_STATUS_ID,
                       callback, user_data);
    break;
  }
  ...
  case RMW_EVENT_PUBLISHER_INCOMPATIBLE_TYPE: {
    auto pub_event = static_cast<CddsPublisher *>(rmw_event->data);
    event_set_callback(pub_event, DDS_INCONSISTENT_TOPIC_STATUS_ID, callback,
                       user_data);
    break;
  }
  ...
  case RMW_EVENT_SUBSCRIPTION_INCOMPATIBLE_TYPE: {
    auto sub_event = static_cast<CddsSubscription *>(rmw_event->data);
    event_set_callback(sub_event, DDS_INCONSISTENT_TOPIC_STATUS_ID, callback,
                       user_data);
    break;
  }
  ...
  case RMW_EVENT_PUBLICATION_MATCHED: {
    auto pub_event = static_cast<CddsPublisher *>(rmw_event->data);
    event_set_callback(pub_event, DDS_PUBLICATION_MATCHED_STATUS_ID, callback,
                       user_data);
    break;
  }
...
extern "C" rmw_ret_t rmw_init_options_init(rmw_init_options_t *init_options,
  ...
  return rmw_discovery_options_init(&(init_options->discovery_options), 0,
                                    &allocator);
}

extern "C" rmw_ret_t rmw_init_options_copy(const rmw_init_options_t *src,
  ...
  RMW_CHECK_TYPE_IDENTIFIERS_MATCH(init options copy,
  ...
  ret = rmw_security_options_copy(&src->security_options, allocator,
...
extern "C" rmw_ret_t rmw_init_options_fini(rmw_init_options_t *init_options) {
  ...
  RMW_CHECK_TYPE_IDENTIFIERS_MATCH(init options,
...
static void convert_guid_to_gid(const dds_guid_t &guid, rmw_gid_t &gid) {
  static_assert(RMW_GID_STORAGE_SIZE >= sizeof(guid),
...
parse_user_data(const dds_qos_t *qos) {
  std::map<std::string, std::vector<uint8_t>> map;
  void *ud;
  size_t udsz;
  if (dds_qget_userdata(qos, &ud, &udsz)) {
    std::vector<uint8_t> udvec(static_cast<uint8_t *>(ud),
                               static_cast<uint8_t *>(ud) + udsz);
    dds_free(ud);
...
static bool get_user_data_key(const dds_qos_t *qos, const std::string key,
                              std::string &value) {
  if (qos != nullptr) {
...
static void handle_ParticipantEntitiesInfo(dds_entity_t reader, void *arg) {
  ...
  bool taken;
  while (rmw_take(impl->common.sub, &msg, &taken, nullptr) == RMW_RET_OK &&
         taken) {
    ...
    if (si.instance_state != DDS_ALIVE_INSTANCE_STATE) {
      ...
      if (dds_qget_userdata(s->qos, &userdata, &userdata_size)) {
        ...
        if (RMW_RET_OK != rmw_dds_common::parse_type_hash_from_user_data(
                              ...
                              userdata_size, type_hash)) {
          RCUTILS_LOG_WARN_NAMED("rmw_cyclonedds_cpp",
                                 "Failed to parse type hash for topic '%s' "
                                 "with type '%s' from USER_DATA '%*s'.",
                                 s->topic_name, s->type_name,
                                 static_cast<int>(userdata_size),
        ...
        if (RMW_RET_OK == rmw_dds_common::parse_sertype_hash_from_user_data(
                              reinterpret_cast<const uint8_t *>(userdata),
                              userdata_size, ser_type_hash)) {
          ser_type_hash_ptr = &ser_type_hash;
        }
      }

      impl->common.graph_cache.add_entity(
          gid, std::string(s->topic_name), std::string(s->type_name), type_hash,
          ppgid, qos_profile, is_reader, ser_type_hash_ptr);
    }
    dds_return_loan(reader, &raw, 1);
  }
}
...
static void discovery_thread(rmw_context_impl_t *impl) {
  const CddsSubscription *sub =
      static_cast<const CddsSubscription *>(impl->common.sub->data);
  const CddsGuardCondition *gc = static_cast<const CddsGuardCondition *>(
      impl->common.listener_thread_gc->data);
  dds_entity_t ws;
  /* deleting ppant will delete waitset as well, so there is no real need to
     delete the waitset here on error, but it is more hygienic */
  if ((ws = dds_create_waitset(DDS_CYCLONEDDS_HANDLE)) < 0) {
    RCUTILS_SAFE_FWRITE_TO_STDERR("ros discovery info listener thread: failed "
                                  "to create waitset, will shutdown ...\n");
    return;
  }
  ...
  std::vector<std::pair<
      dds_entity_t, std::function<void(dds_entity_t, rmw_context_impl_t *)>>>
      entries = {
          {gc->gcondh, nullptr},
  ...
  for (size_t i = 0; i < entries.size(); i++) {
    if (entries[i].second != nullptr &&
      ...
      RCUTILS_SAFE_FWRITE_TO_STDERR("ros discovery info listener thread: "
                                    "failed to set reader status masks, "
                                    "will shutdown ...\n");
      return;
    }
    if (dds_waitset_attach(ws, entries[i].first, static_cast<dds_attach_t>(i)) <
        0) {
      RCUTILS_SAFE_FWRITE_TO_STDERR("ros discovery info listener thread: "
                                    "failed to attach entities to waitset, "
                                    "will shutdown ...\n");
      dds_delete(ws);
      return;
    }
  ...
  while (impl->common.thread_is_running.load()) {
    ...
    if ((n = dds_waitset_wait(ws, xs.data(), xs.size(), DDS_INFINITY)) < 0) {
      RCUTILS_SAFE_FWRITE_TO_STDERR("ros discovery info listener thread: wait "
                                    "failed, will shutdown ...\n");
      return;
    }
...
static rmw_ret_t discovery_thread_start(rmw_context_impl_t *impl) {
  ...
  if (common_context->listener_thread_gc) {
    try {
      ...
      RMW_SET_ERROR_MSG_WITH_FORMAT_STRING("Failed to create std::thread: %s",
                                           exc.what());
    } catch (...) {
  ...
  if (common_context->listener_thread_gc) {
    if (RMW_RET_OK !=
        destroy_guard_condition(common_context->listener_thread_gc)) {
      RCUTILS_SAFE_FWRITE_TO_STDERR(
          RCUTILS_STRINGIFY(__FILE__) ":" RCUTILS_STRINGIFY(
              __function__) ":" RCUTILS_STRINGIFY(__LINE__) ": Failed to "
                                                            "destroy guard "
                                                            "condition");
    }
  }
...
static rmw_ret_t
discovery_thread_stop(rmw_dds_common::Context &common_context) {
  if (common_context.thread_is_running.exchange(false)) {
    rmw_ret_t rmw_ret =
        rmw_trigger_guard_condition(common_context.listener_thread_gc);
    ...
    try {
      ...
      RMW_SET_ERROR_MSG_WITH_FORMAT_STRING("Failed to join std::thread: %s",
                                           exc.what());
      return RMW_RET_ERROR;
    } catch (...) {
...
static bool check_create_domain(dds_domainid_t did,
                                rmw_discovery_options_t *discovery_options) {
  std::lock_guard<std::mutex> lock(gcdds().domains_lock);
  /* return true: n_nodes incremented, discovery params set correctly, domain
     exists "      false: n_nodes unchanged, domain left intact if it already
     existed */
  CddsDomain &dom = gcdds().domains[did];
  if (dom.refcount != 0) {
    /* Discovery parameters must match */
    bool options_equal = false;
    const auto rc = rmw_discovery_options_equal(
        discovery_options, &dom.discovery_options, &options_equal);
    if (RMW_RET_OK != rc) {
      RCUTILS_LOG_ERROR_NAMED("rmw_cyclonedds_cpp",
                              "check_create_domain: unable to check if "
                              "discovery options are equal: %i",
                              rc);
      return false;
    }
    if (options_equal) {
      ...
      RCUTILS_LOG_ERROR_NAMED("rmw_cyclonedds_cpp",
                              "check_create_domain: attempt at creating nodes "
                              "in the same domain with different "
    ...
    case RMW_AUTOMATIC_DISCOVERY_RANGE_SYSTEM_DEFAULT:
      ...
      if (discovery_options->static_peers_count > 0) {
        RCUTILS_LOG_WARN_NAMED("rmw_cyclonedds_cpp",
                               "check_create_domain: %lu static peers were "
                               "specified, but discovery is "
                               "set to use the RMW implementation default, so "
                               "these static peers will be ignored.",
    ...
    case RMW_AUTOMATIC_DISCOVERY_RANGE_OFF:
      ...
      if (discovery_options->static_peers_count > 0) {
        RCUTILS_LOG_WARN_NAMED(
            "rmw_cyclonedds_cpp",
            "check_create_domain: %lu static peers were specified, but "
            "discovery is "
    ...
    if (add_localhost_as_static_peer || add_static_peers || disable_multicast) {
      ...
      const bool discovery_off = disable_multicast &&
      ...
      if (discovery_off) {
        /* This means we have an OFF range, so we should use the domain tag to
          ...
          Setting ParticipantIndex to none eliminates the 119 limit on the
          number of participants on a machine.
          */
        config += "<Discovery><ParticipantIndex>none</ParticipantIndex>";
        config += "<Tag>ros_discovery_off_" +
                  std::to_string(rcutils_get_pid()) + "</Tag>";
      } else {
        config += "<Discovery><ParticipantIndex>auto</ParticipantIndex>";
        // This controls the number of participants that can be discovered on a
        // single host, which is roughly equivalent to the number of ROS 2
        // processes. If it's too small then we won't connect to all
        // participants. If it's too large then we will send a lot of
        // announcement traffic. The default number here is picked arbitrarily.
    ...
    if ((get_env_error = rcutils_get_env("CYCLONEDDS_URI", &config_from_env)) ==
        nullptr) {
      config += std::string(config_from_env);
    } else {
      RCUTILS_LOG_ERROR_NAMED("rmw_cyclonedds_cpp",
                              "rmw_create_node: failed to retrieve "
                              "CYCLONEDDS_URI environment variable, error %s",
                              get_env_error);
      gcdds().domains.erase(did);
      return false;
    }

    RCUTILS_LOG_DEBUG_NAMED("rmw_cyclonedds_cpp", "Config XML is %s",
                            config.c_str());

    if ((dom.domain_handle = dds_create_domain(did, config.c_str())) < 0) {
      RCUTILS_LOG_ERROR_NAMED(
          "rmw_cyclonedds_cpp",
          "rmw_create_node: failed to create domain, error %s",
...
static rmw_ret_t
configure_qos_for_security(dds_qos_t *qos,
                           const rmw_security_options_t *security_options) {
#if RMW_SUPPORT_SECURITY
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rcutils_string_map_t security_files =
  ...
  auto scope_exit_ws = rcpputils::make_scope_exit([&security_files]() {
  ...
  if (get_security_files("file:", security_options->security_root_path,
                         &security_files) != RMW_RET_OK) {
    RCUTILS_LOG_INFO_NAMED("rmw_cyclonedds_cpp",
                           "could not find all security files");
    return RMW_RET_UNSUPPORTED;
  }
  ...
  dds_qset_prop(qos, "dds.sec.auth.library.finalize",
  ...
  dds_qset_prop(qos, "dds.sec.access.library.finalize",
  ...
  if (security_options->enforce_security == RMW_SECURITY_ENFORCEMENT_ENFORCE) {
    RMW_SET_ERROR_MSG("Security was requested but the Cyclone DDS being used "
                      "does not have security "
                      "support enabled. Recompile Cyclone DDS with the "
                      "'-DENABLE_SECURITY=ON' "
                      "CMake option");
  }
  return RMW_RET_UNSUPPORTED;
...
rmw_ret_t rmw_context_impl_s::init(rmw_init_options_t *options,
                                   size_t domain_id) {
  ...
  /* Take domains_lock and hold it until after the participant creation
    succeeded or failed: otherwise there is a race with rmw_destroy_node
    deleting the last participant and tearing down the domain for versions of
    Cyclone that implement the original version of dds_create_domain that
    doesn't return a handle.  */
  ...
  std::string user_data = std::string("enclave=") +
                          std::string(options->enclave) + std::string(";");
  dds_qset_userdata(ppant_qos.get(), user_data.c_str(), user_data.size());
  if (configure_qos_for_security(ppant_qos.get(), &options->security_options) !=
      RMW_RET_OK) {
    if (RMW_SECURITY_ENFORCEMENT_ENFORCE ==
  ...
  this->ppant =
      dds_create_participant(this->domain_id, ppant_qos.get(), nullptr);
  if (this->ppant < 0) {
    this->clean_up();
    RCUTILS_LOG_ERROR_NAMED(
        "rmw_cyclonedds_cpp",
  ...
  if ((this->rd_participant =
           dds_create_reader(this->ppant, DDS_BUILTIN_TOPIC_DCPSPARTICIPANT,
                             nullptr, nullptr)) < 0) {
    this->clean_up();
    RCUTILS_LOG_ERROR_NAMED(
        "rmw_cyclonedds_cpp",
        "rmw_create_node: failed to create DCPSParticipant reader");
    return RMW_RET_ERROR;
  }
  if ((this->rd_subscription =
           dds_create_reader(this->ppant, DDS_BUILTIN_TOPIC_DCPSSUBSCRIPTION,
                             nullptr, nullptr)) < 0) {
    this->clean_up();
    RCUTILS_LOG_ERROR_NAMED(
        "rmw_cyclonedds_cpp",
        "rmw_create_node: failed to create DCPSSubscription reader");
    return RMW_RET_ERROR;
  }
  if ((this->rd_publication =
           dds_create_reader(this->ppant, DDS_BUILTIN_TOPIC_DCPSPUBLICATION,
                             nullptr, nullptr)) < 0) {
    this->clean_up();
    RCUTILS_LOG_ERROR_NAMED(
        "rmw_cyclonedds_cpp",
        "rmw_create_node: failed to create DCPSPublication reader");
    return RMW_RET_ERROR;
  }
  /* Create DDS publisher/subscriber objects that will be used for all DDS
    writers/readers to be created for RMW publishers/subscriptions. */
  if ((this->dds_pub = dds_create_publisher(this->ppant, nullptr, nullptr)) <
      0) {
    this->clean_up();
    RCUTILS_LOG_ERROR_NAMED("rmw_cyclonedds_cpp",
  ...
  if ((this->dds_sub = dds_create_subscriber(this->ppant, nullptr, nullptr)) <
      0) {
    this->clean_up();
    RCUTILS_LOG_ERROR_NAMED("rmw_cyclonedds_cpp",
  ...
  this->common.pub =
      create_publisher(this->ppant, this->dds_pub,
  ...
  this->common.publish_callback = [](const rmw_publisher_t *pub,
                                     const void *msg) {
    return rmw_publish(pub, msg, nullptr);
  };
  ...
  this->common.graph_cache.set_on_change_callback(
      [guard_condition = this->common.graph_guard_condition]() {
        ...
        if (ret != RMW_RET_OK) {
          RMW_SET_ERROR_MSG("graph cache on_change_callback failed to trigger "
                            "guard condition");
        }
      });
  ...
  // One could also use a set of listeners instead of a thread for maintaining
  // the graph cache:
  // - Locally published samples shouldn't make it to the reader, so there
  // shouldn't be a deadlock
  //   caused by the graph cache's mutex already having been locked by (e.g.)
  //   rmw_create_node.
  // - Whatever the graph cache implementation does, it shouldn't involve much
  // more than local state
  //   updates and triggering a guard condition, and so that should be safe.
  // however, the graph cache updates could be expensive, and so performing
  // those operations on the thread receiving data from the network may not be
  // wise.
  rmw_ret_t ret;
...
static void *init_and_alloc_sample(entityT &entity, const uint32_t sample_size,
                                   const bool alloc_on_heap = false) {
  // initialise the data allocator
  if (alloc_on_heap) {
    if (dds_data_allocator_init_heap(&entity->data_allocator) !=
        DDS_RETCODE_OK) {
      RMW_SET_ERROR_MSG("Reader data allocator initialization failed for heap");
      return nullptr;
    }
  } else {
    if (dds_data_allocator_init(entity->enth, &entity->data_allocator) !=
        DDS_RETCODE_OK) {
  ...
  auto chunk_ptr =
      dds_data_allocator_alloc(&entity->data_allocator, sample_size);
  RMW_CHECK_FOR_NULL_WITH_MSG(chunk_ptr, "Failed to get loan", return nullptr);
  // Don't initialize the message memory, as this allocated memory will anyways
  // be filled by the user and initializing the memory here just creates
  // undesired performance hit with the zero-copy path
  return chunk_ptr;
}
...
static rmw_ret_t fini_and_free_sample(entityT &entity, void *loaned_message) {
  ...
  if (dds_data_allocator_free(&entity->data_allocator, loaned_message) !=
      DDS_RETCODE_OK) {
...
extern "C" rmw_ret_t rmw_init(const rmw_init_options_t *options,
  ...
  if (options->domain_id >= UINT32_MAX &&
    ...
    RCUTILS_LOG_ERROR_NAMED("rmw_cyclonedds_cpp",
                            "rmw_init: domain id out of range");
    return RMW_RET_INVALID_ARGUMENT;
  }
  ...
  // No custom handling of RMW_DEFAULT_DOMAIN_ID. Simply use a reasonable domain
  // id.
  ...
  auto cleanup_impl =
      rcpputils::make_scope_exit([context]() { delete context->impl; });
...
extern "C" rmw_node_t *rmw_create_node(rmw_context_t *context, const char *name,
  ...
  auto finalize_context =
      rcpputils::make_scope_exit([context]() { context->impl->fini(); });
  ...
  node->namespace_ = static_cast<const char *>(
      rmw_allocate(sizeof(char) * strlen(namespace_) + 1));
  RET_ALLOC_X(node->namespace_, return nullptr);
  memcpy(const_cast<char *>(node->namespace_), namespace_,
         strlen(namespace_) + 1);

  auto common = &context->impl->common;
  rmw_ret_t rmw_ret = common->add_node_graph(name, namespace_);
  if (RMW_RET_OK != rmw_ret) {
    return nullptr;
  }
...
extern "C" rmw_ret_t rmw_destroy_node(rmw_node_t *node) {
  ...
  result_ret = common->remove_node_graph(node->name, node->namespace_);
...
using MessageTypeSupport_c = rmw_cyclonedds_cpp::MessageTypeSupport<
    rosidl_typesupport_introspection_c__MessageMembers>;
using MessageTypeSupport_cpp = rmw_cyclonedds_cpp::MessageTypeSupport<
...
rmw_deserialize(const rmw_serialized_message_t *serialized_message,
  ...
  thread_local const rosidl_message_type_support_t *cached_resolved_ts =
      nullptr;
...
/* Publications need the sertype that DDSI uses for the topic when publishing a
   serialized message.  With the old ("arbitrary") interface of Cyclone, one
   doesn't know the sertype that is actually used because that may be the one
   that was provided in the call to dds_create_topic_arbitrary(), but it may
   also be one that was introduced by a preceding call to create the same topic.

   There is no way of discovering which case it is, and there is no way of
   getting access to the correct sertype.  The best one can do is to keep using
   one provided when creating the topic -- and fortunately using the wrong
   sertype has surprisingly few nasty side-effects, but it still wrong.

   Because the caller retains ownership, so this is easy, but it does require
   dropping the reference when cleaning up.

   The new ("generic") interface instead takes over the ownership of the
   reference iff it succeeds and it returns a non-counted reference to the
   sertype actually used.  The lifetime of the reference is at least as long as
   the lifetime of the DDS topic exists; and the topic's lifetime is at least
   that of the readers/writers using it.  This reference can therefore safely be
   used. */

static dds_entity_t create_topic(dds_entity_t pp, const char *name,
                                 struct ddsi_sertype *sertype,
...
void set_error_message_from_create_topic(dds_entity_t topic,
  ...
  if (DDS_RETCODE_BAD_PARAMETER == topic) {
    ...
    const std::string error_msg = "failed to create topic [" + topic_name +
                                  "] because it's already in use in this "
                                  "context with incompatible QoS settings";
    RMW_SET_ERROR_MSG(error_msg.c_str());
  } else if (DDS_RETCODE_PRECONDITION_NOT_MET == topic) {
    const std::string error_msg = "failed to create topic [" + topic_name +
                                  "] because it's already in use in this "
                                  "context with a different message type";
    RMW_SET_ERROR_MSG(error_msg.c_str());
  } else {
    const std::string error_msg =
        "failed to create topic [" + topic_name + "] for unknown reasons";
    RMW_SET_ERROR_MSG(error_msg.c_str());
  }
}
...
extern "C" rmw_ret_t rmw_publish(const rmw_publisher_t *publisher,
  ...
  TRACETOOLS_TRACEPOINT(rmw_publish, (const void *)publisher, ros_message,
                        tstamp);
...
extern "C" rmw_ret_t rmw_publish_serialized_message(
  ...
  RMW_CHECK_TYPE_IDENTIFIERS_MATCH(publisher,
                                   ...
                                   eclipse_cyclonedds_identifier,
                                   return RMW_RET_INCORRECT_RMW_IMPLEMENTATION);
  RMW_CHECK_FOR_NULL_WITH_MSG(serialized_message,
                              "serialized message handle is null",
                              return RMW_RET_INVALID_ARGUMENT);
  auto pub = static_cast<CddsPublisher *>(publisher->data);
  const dds_time_t tstamp = dds_time();
  TRACETOOLS_TRACEPOINT(rmw_publish, (const void *)publisher,
                        serialized_message, tstamp);

  struct ddsi_serdata *d = serdata_rmw_from_serialized_message(
      pub->sertype, serialized_message->buffer,
  ...
  if (dds_is_shared_memory_available(pub->enth)) {
    auto sample_ptr =
        init_and_alloc_sample(pub, serialized_message->buffer_length);
    RET_NULL_X(sample_ptr, return RMW_RET_ERROR);
    memcpy(sample_ptr, serialized_message->buffer,
           serialized_message->buffer_length);
...
static rmw_ret_t publish_loaned_int(const rmw_publisher_t *publisher,
                                    void *ros_message) {
#ifdef DDS_HAS_SHM
  RMW_CHECK_FOR_NULL_WITH_MSG(publisher, "publisher handle is null",
                              return RMW_RET_INVALID_ARGUMENT);
  ...
  if (cdds_publisher->is_loaning_available) {
    auto d =
        new serdata_rmw(cdds_publisher->sertype, ddsi_serdata_kind::SDK_DATA);
    ...
    RMW_SET_ERROR_MSG(
        "Publishing a loaned message of non fixed type is not allowed");
    return RMW_RET_ERROR;
  }
  return RMW_RET_OK;
#else
...
extern "C" rmw_ret_t
rmw_publish_loaned_message(const rmw_publisher_t *publisher, void *ros_message,
                           rmw_publisher_allocation_t *allocation) {
...
get_typesupport(const rosidl_message_type_support_t *type_supports) {
...
static std::string make_fqtopic(const char *prefix, const char *topic_name,
                                const char *suffix,
...
static std::string make_fqtopic(const char *prefix, const char *topic_name,
                                const char *suffix,
                                const rmw_qos_profile_t *qos_policies) {
  return make_fqtopic(prefix, topic_name, suffix,
...
static dds_qos_t *create_readwrite_qos(const rmw_qos_profile_t *qos_policies,
  ...
  case RMW_QOS_POLICY_HISTORY_KEEP_LAST:
    if (qos_policies->depth == RMW_QOS_POLICY_DEPTH_SYSTEM_DEFAULT) {
      ...
      dds_qset_history(qos, DDS_HISTORY_KEEP_LAST,
                       static_cast<int32_t>(qos_policies->depth));
    }
    break;
  ...
  case RMW_QOS_POLICY_DURABILITY_TRANSIENT_LOCAL: {
    /* Cyclone uses durability service QoS for determining what to retain as
       historical data, separating the reliability window from the historical
       data; but that is somewhat unusual among DDS implementations ... */
...
static rmw_qos_policy_kind_t
dds_qos_policy_to_rmw_qos_policy(dds_qos_policy_id_t policy_id) {
  switch (policy_id) {
...
static bool dds_qos_to_rmw_qos(const dds_qos_t *dds_qos,
...

  {
    ...
    qos_policies->liveliness_lease_duration =
        dds_duration_to_rmw(lease_duration);
  }
...
static bool
is_type_self_contained(const rosidl_message_type_support_t *type_supports) {
...
static CddsPublisher *
create_cdds_publisher(dds_entity_t dds_ppant, dds_entity_t dds_pub,
                      const rosidl_message_type_support_t *type_supports,
                      const char *topic_name,
                      const rmw_qos_profile_t *qos_policies) {
  ...
  std::string fqtopic_name =
      make_fqtopic(ROS_TOPIC_PREFIX, topic_name, "", qos_policies);
  bool is_fixed_type = is_type_self_contained(type_support);
  uint32_t sample_size =
      static_cast<uint32_t>(rmw_cyclonedds_cpp::get_message_size(type_support));
  auto sertype = create_sertype(
      type_support->typesupport_identifier,
      create_message_type_support(type_support->data,
                                  type_support->typesupport_identifier),
      false, rmw_cyclonedds_cpp::make_message_value_type(type_supports),
...
fail_instance_handle:
  if (dds_delete(pub->enth) < 0) {
    RCUTILS_LOG_ERROR_NAMED("rmw_cyclonedds_cpp",
                            "failed to destroy writer during error handling");
  }
fail_writer:
  dds_delete_qos(qos);
fail_qos:
...
create_publisher(dds_entity_t dds_ppant, dds_entity_t dds_pub,
                 ...
                 const rmw_publisher_options_t *publisher_options) {
  CddsPublisher *pub;
  if ((pub = create_cdds_publisher(dds_ppant, dds_pub, type_supports,
                                   topic_name, qos_policies)) == nullptr) {
    return nullptr;
  }
  auto cleanup_cdds_publisher = rcpputils::make_scope_exit([pub]() {
    if (dds_delete(pub->enth) < 0) {
      RCUTILS_LOG_ERROR_NAMED("rmw_cyclonedds_cpp",
                              "failed to delete writer during error handling");
    }
    delete pub;
  });
  ...
  rmw_publisher->topic_name =
      reinterpret_cast<char *>(rmw_allocate(strlen(topic_name) + 1));
  RET_ALLOC_X(rmw_publisher->topic_name, return nullptr);
  memcpy(const_cast<char *>(rmw_publisher->topic_name), topic_name,
         strlen(topic_name) + 1);
...
extern "C" rmw_publisher_t *rmw_create_publisher(
  ...
  rmw_publisher_t *pub = create_publisher(
      node->context->impl->ppant, node->context->impl->dds_pub, type_supports,
      topic_name, &adapted_qos_policies, publisher_options);
  if (pub == nullptr) {
    return nullptr;
  }
  auto cleanup_publisher = rcpputils::make_scope_exit([pub]() {
    ...
    if (RMW_RET_OK != destroy_publisher(pub)) {
      ...
      RMW_SAFE_FWRITE_TO_STDERR(
          " during '" RCUTILS_STRINGIFY(__function__) "' cleanup\n");
      rmw_reset_error();
    }
    rmw_set_error_state(error_state.message, error_state.file,
  ...
  if (RMW_RET_OK !=
      common->add_publisher_graph(cddspub->gid, node->name, node->namespace_)) {
    return nullptr;
  }

  cleanup_publisher.cancel();
  TRACETOOLS_TRACEPOINT(rmw_publisher_init, static_cast<const void *>(pub),
                        cddspub->gid.data);
  return pub;
}

extern "C" rmw_ret_t rmw_get_gid_for_publisher(const rmw_publisher_t *publisher,
                                               rmw_gid_t *gid) {
  ...
  static_assert(sizeof(pub->pubiid) <= sizeof(gid->data),
                "publisher id is larger than max rmw gid size");
  memcpy(gid->data, &pub->pubiid, sizeof(pub->pubiid));
  return RMW_RET_OK;
}

extern "C" rmw_ret_t rmw_get_gid_for_client(const rmw_client_t *client,
                                            rmw_gid_t *gid) {
  ...
  gid->implementation_identifier = eclipse_cyclonedds_identifier;
  memset(gid->data, 0, sizeof(gid->data));
  static_assert(sizeof(cli->client.id.data) <= sizeof(gid->data),
                "client id is larger than max rmw gid size");
  memcpy(gid->data, cli->client.id.data, sizeof(cli->client.id.data));
  return RMW_RET_OK;
}

extern "C" rmw_ret_t rmw_compare_gids_equal(const rmw_gid_t *gid1,
  ...
  /* alignment is potentially lost because of the translation to an array of
     bytes, so use memcmp instead of a simple integer comparison */
...
static rmw_ret_t
borrow_loaned_message_int(const rmw_publisher_t *publisher,
                          const rosidl_message_type_support_t *type_support,
                          void **ros_message) {
#ifdef DDS_HAS_SHM
  ...
  if (cdds_publisher->is_loaning_available) {
    auto sample_ptr =
        init_and_alloc_sample(cdds_publisher, cdds_publisher->sample_size);
    RET_NULL_X(sample_ptr, return RMW_RET_ERROR);
  ...
  (void)ros_message;
  RMW_SET_ERROR_MSG(
      "rmw_borrow_loaned_message not implemented for rmw_cyclonedds_cpp");
  return RMW_RET_UNSUPPORTED;
#endif
}
...
static rmw_ret_t
return_loaned_message_from_publisher_int(const rmw_publisher_t *publisher,
                                         void *loaned_message) {
#ifdef DDS_HAS_SHM
  RCUTILS_CHECK_ARGUMENT_FOR_NULL(publisher, RMW_RET_INVALID_ARGUMENT);
  if (!publisher->can_loan_messages) {
...
extern "C" rmw_ret_t rmw_destroy_publisher(rmw_node_t *node,
  ...
  rmw_ret_t publish_ret = common->remove_publisher_graph(
      cddspub->gid, node->name, node->namespace_);
  ...
  if (RMW_RET_OK != inner_ret) {
    if (RMW_RET_OK != ret) {
      RMW_SAFE_FWRITE_TO_STDERR(rmw_get_error_string().str);
      RMW_SAFE_FWRITE_TO_STDERR(
          " during '" RCUTILS_STRINGIFY(__function__) "'\n");
    } else {
      error_state = *rmw_get_error_state();
      ret = inner_ret;
    }
...
static CddsSubscription *create_cdds_subscription(
  ...
  dds_lset_data_available_arg(listener, dds_listener_callback,
                              &sub->user_callback_data, false);
  // Set the corresponding callbacks to listen for events
...
fail_readcond:
  if (dds_delete(sub->enth) < 0) {
    RCUTILS_LOG_ERROR_NAMED("rmw_cyclonedds_cpp",
                            "failed to delete reader during error handling");
  }
fail_reader:
  dds_delete_qos(qos);
fail_qos:
...
static rmw_subscription_t *
create_subscription(dds_entity_t dds_ppant, dds_entity_t dds_sub,
                    const rosidl_message_type_support_t *type_supports,
                    const char *topic_name,
                    const rmw_qos_profile_t *qos_policies,
                    const rmw_subscription_options_t *subscription_options) {
  ...
  auto cleanup_subscription = rcpputils::make_scope_exit([sub]() {
    if (dds_delete(sub->rdcondh) < 0) {
      RMW_SAFE_FWRITE_TO_STDERR(
          "failed to delete readcondition during '" RCUTILS_STRINGIFY(
              __function__) "' cleanup\n");
    }
    if (dds_delete(sub->enth) < 0) {
      RMW_SAFE_FWRITE_TO_STDERR(
          "failed to delete reader during '" RCUTILS_STRINGIFY(
              __function__) "' cleanup\n");
    }
    delete sub;
  });
  ...
  auto cleanup_rmw_subscription =
      rcpputils::make_scope_exit([rmw_subscription]() {
...
extern "C" rmw_subscription_t *rmw_create_subscription(
  ...
  if (!qos_policies->avoid_ros_namespace_conventions) {
    ...
    if (RMW_TOPIC_VALID != validation_result) {
      const char *reason =
          rmw_full_topic_name_validation_result_string(validation_result);
      RMW_SET_ERROR_MSG_WITH_FORMAT_STRING("invalid topic_name argument: %s",
                                           reason);
      return nullptr;
    }
  }
  ...
  rmw_subscription_t *sub = create_subscription(
      ...
      topic_name, &adapted_qos_policies, subscription_options);
  if (sub == nullptr) {
    return nullptr;
  }
  auto cleanup_subscription = rcpputils::make_scope_exit([sub]() {
  ...
  if (RMW_RET_OK != common->add_subscriber_graph(cddssub->gid, node->name,
                                                 node->namespace_)) {
    return nullptr;
  }

  cleanup_subscription.cancel();
  TRACETOOLS_TRACEPOINT(rmw_subscription_init, static_cast<const void *>(sub),
                        cddssub->gid.data);
  return sub;
}
...
rmw_destroy_subscription(rmw_node_t *node, rmw_subscription_t *subscription) {
  ...
  auto common = &node->context->impl->common;
  const auto cddssub =
      static_cast<const CddsSubscription *>(subscription->data);
  ret = common->remove_publisher_graph(cddssub->gid, node->name,
                                       node->namespace_);
  if (RMW_RET_OK != ret) {
    error_state = *rmw_get_error_state();
...
static void message_info_from_sample_info(const dds_sample_info_t &info,
                                          rmw_message_info_t *message_info) {
  message_info->publisher_gid.implementation_identifier =
      eclipse_cyclonedds_identifier;
  memset(message_info->publisher_gid.data, 0,
         sizeof(message_info->publisher_gid.data));
  assert(sizeof(info.publication_handle) <=
         sizeof(message_info->publisher_gid.data));
...
static rmw_ret_t rmw_take_int(const rmw_subscription_t *subscription,
  ...
  *taken = false;
take_done:
  TRACETOOLS_TRACEPOINT(rmw_take, static_cast<const void *>(subscription),
...
static rmw_ret_t
rmw_take_seq(const rmw_subscription_t *subscription, size_t count,
  ...
  auto ret = dds_take(sub->enth, message_sequence->data, infos.data(), count,
                      maxsamples);

  // Returning 0 should not be an error, as it just indicates that no messages
  // were available.
  if (ret < 0) {
    return RMW_RET_ERROR;
  }

  // Keep track of taken/not taken to reorder sequence with valid messages at
  // the front
...
static rmw_ret_t rmw_take_ser_int(const rmw_subscription_t *subscription,
                                  rmw_serialized_message_t *serialized_message,
                                  bool *taken,
                                  rmw_message_info_t *message_info) {
  RMW_CHECK_ARGUMENT_FOR_NULL(subscription, RMW_RET_INVALID_ARGUMENT);
    ...
    if (info.valid_data) {
      ...
      if (d->iox_chunk != nullptr) {
        ...
        if (iox_header->shm_data_state == IOX_CHUNK_CONTAINS_SERIALIZED_DATA) {
          ...
          if (rmw_serialized_message_resize(serialized_message, size) !=
              RMW_RET_OK) {
            ddsi_serdata_unref(d);
            *taken = false;
            return RMW_RET_ERROR;
          ...
          *taken = true;
          TRACETOOLS_TRACEPOINT(
              rmw_take, static_cast<const void *>(subscription),
          ...
          if (rmw_serialize(d->iox_chunk, &sub->type_supports,
        ...
        // release the chunk
        free_iox_chunk(static_cast<iox_sub_t *>(d->iox_subscriber),
                       &d->iox_chunk);
      } else // NOLINT
#endif
      {
  ...
  *taken = false;
  TRACETOOLS_TRACEPOINT(rmw_take, static_cast<const void *>(subscription),
                        static_cast<const void *>(serialized_message), 0LL,
                        *taken);
  return RMW_RET_OK;
}

static rmw_ret_t rmw_take_loan_int(const rmw_subscription_t *subscription,
  ...
  dds_sample_info_t info;
  struct ddsi_serdata *d;
  while (dds_takecdr(cdds_subscription->enth, &d, 1, &info, DDS_ANY_STATE) ==
         1) {
    if (info.valid_data) {
      if (message_info) {
        message_info_from_sample_info(info, message_info);
      ...
      if (d->iox_chunk != nullptr) {
        // the iox chunk has data, based on the kind of the data return the data
        ...
        if (iox_header->shm_data_state == IOX_CHUNK_CONTAINS_SERIALIZED_DATA) {
          ...
          if (rmw_deserialize(&ser_msg, &cdds_subscription->type_supports,
                              *loaned_message) != RMW_RET_OK) {
            RMW_SET_ERROR_MSG(
                "Failed to deserialize sample from shared memory buffer");
            ddsi_serdata_unref(d);
            *taken = false;
            return RMW_RET_ERROR;
          }
        ...
        *taken = true;
        // doesn't allocate, but initialise the allocator to free the chunk
        // later when the loan is returned
        dds_data_allocator_init(cdds_subscription->enth,
                                &cdds_subscription->data_allocator);
        // set the loaned chunk to null, so that the  loaned chunk is not
        // release in rmw_serdata_free(), but will be released when
        ...
        auto sample_ptr =
            init_and_alloc_sample(cdds_subscription, d->type->iox_size, true);
        RET_NULL_X(sample_ptr, return RMW_RET_ERROR);
  ...
  *taken = false;
  ...
  RMW_SET_ERROR_MSG(
      "rmw_take_loaned_message not implemented for rmw_cyclonedds_cpp");
  return RMW_RET_UNSUPPORTED;
#endif
}

extern "C" rmw_ret_t rmw_take(const rmw_subscription_t *subscription,
                              void *ros_message, bool *taken,
                              rmw_subscription_allocation_t *allocation) {
  static_cast<void>(allocation);
...
extern "C" rmw_ret_t
rmw_take_sequence(const rmw_subscription_t *subscription, size_t count,
                  rmw_message_sequence_t *message_sequence,
                  rmw_message_info_sequence_t *message_info_sequence,
                  size_t *taken, rmw_subscription_allocation_t *allocation) {
  static_cast<void>(allocation);
  return rmw_take_seq(subscription, count, message_sequence,
                      message_info_sequence, taken);
}
...
extern "C" rmw_ret_t rmw_take_serialized_message_with_info(
  ...
  return rmw_take_ser_int(subscription, serialized_message, taken,
                          message_info);
}
...
static rmw_ret_t return_loaned_message_from_subscription_int(
  ...
  (void)subscription;
  (void)loaned_message;
  RMW_SET_ERROR_MSG("rmw_return_loaned_message_from_subscription not "
                    "implemented for rmw_cyclonedds_cpp");
  return RMW_RET_UNSUPPORTED;
#endif
}
...
extern "C" rmw_ret_t rmw_return_loaned_message_from_subscription(
    const rmw_subscription_t *subscription, void *loaned_message) {
  return return_loaned_message_from_subscription_int(subscription,
                                                     loaned_message);
}
...
static rmw_ret_t init_rmw_event(rmw_event_t *rmw_event,
                                ...
                                void *data, rmw_event_type_t event_type) {
  RMW_CHECK_ARGUMENT_FOR_NULL(rmw_event, RMW_RET_INVALID_ARGUMENT);
  ...
  if (!is_event_supported(event_type)) {
    RMW_SET_ERROR_MSG(
        "provided event_type is not supported by rmw_cyclonedds_cpp");
    return RMW_RET_UNSUPPORTED;
  }
...
extern "C" rmw_ret_t rmw_publisher_event_init(rmw_event_t *rmw_event,
                                              const rmw_publisher_t *publisher,
                                              rmw_event_type_t event_type) {
  RET_NULL(publisher);
  RMW_CHECK_TYPE_IDENTIFIERS_MATCH(publisher,
  ...
  return init_rmw_event(rmw_event, publisher->implementation_identifier,
                        publisher->data, event_type);
}

extern "C" rmw_ret_t
rmw_subscription_event_init(rmw_event_t *rmw_event,
                            const rmw_subscription_t *subscription,
                            rmw_event_type_t event_type) {
  ...
  RMW_CHECK_TYPE_IDENTIFIERS_MATCH(subscription,
                                   ...
                                   eclipse_cyclonedds_identifier,
                                   return RMW_RET_INCORRECT_RMW_IMPLEMENTATION);
  return init_rmw_event(rmw_event, subscription->implementation_identifier,
...
extern "C" rmw_ret_t rmw_take_event(const rmw_event_t *event_handle,
                                    void *event_info, bool *taken) {
  RET_NULL(event_handle);
  RMW_CHECK_TYPE_IDENTIFIERS_MATCH(event_handle,
  ...
  case RMW_EVENT_REQUESTED_QOS_INCOMPATIBLE: {
    auto ei = static_cast<rmw_requested_qos_incompatible_event_status_t *>(
        event_info);
    auto sub = static_cast<CddsSubscription *>(event_handle->data);
...
static rmw_guard_condition_t *create_guard_condition() {
  ...
  if ((gcond_impl->gcondh = dds_create_guardcondition(DDS_CYCLONEDDS_HANDLE)) <
      0) {
...
static rmw_ret_t
destroy_guard_condition(rmw_guard_condition_t *guard_condition_handle) {
  RET_NULL(guard_condition_handle);
  auto *gcond_impl =
      static_cast<CddsGuardCondition *>(guard_condition_handle->data);
  clean_waitset_caches();
...
extern "C" rmw_wait_set_t *rmw_create_wait_set(rmw_context_t *context,
...

  {
    ...
    if (gcdds().waitsets.size() == 0) {
      if ((gcdds().gc_for_empty_waitset =
               dds_create_guardcondition(DDS_CYCLONEDDS_HANDLE)) < 0) {
        RMW_SET_ERROR_MSG(
            "failed to create guardcondition for handling empty waitsets");
        goto fail_create_dummy;
      }
    }
    // Attach never-triggered guard condition.  As it will never be triggered,
    // it will never be included in the result of dds_waitset_wait
    if (dds_waitset_attach(ws->waitseth, gcdds().gc_for_empty_waitset,
                           INTPTR_MAX) < 0) {
      RMW_SET_ERROR_MSG("failed to attach dummy guard condition for blocking "
                        "on empty waitset");
...
template <typename T>
static bool require_reattach(const std::vector<T *> &cached, size_t count,
                             void **ary) {
  if (ary == nullptr || count == 0) {
    ...
    return memcmp(static_cast<const void *>(cached.data()),
                  static_cast<void *>(ary), count * sizeof(void *)) != 0;
  }
}

static bool require_reattach(const std::vector<CddsEvent> &cached,
                             rmw_events_t *events) {
  if (events == nullptr || events->event_count == 0) {
    ...
    for (size_t i = 0; i < events->event_count; ++i) {
      rmw_event_t *current_event =
          static_cast<rmw_event_t *>(events->events[i]);
...
static void clean_waitset_caches() {
  /* Called whenever a subscriber, guard condition, service or client is deleted
     (as these may have been cached in a waitset), and drops all cached entities
     from all waitsets (just to keep life simple). I'm assuming one is not
     allowed to delete an entity while it is still being used ... */
  std::lock_guard<std::mutex> lock(gcdds().lock);
...
static rmw_ret_t
gather_event_entities(const rmw_events_t *events,
  ...
  for (size_t i = 0; i < events->event_count; ++i) {
    ...
    dds_entity_t dds_entity =
        static_cast<CddsEntity *>(current_event->data)->enth;
    if (dds_entity <= 0) {
    ...
    if (is_event_supported(current_event->event_type)) {
      ...
      uint32_t status_kind =
          get_status_kind_from_rmw(current_event->event_type);
      // TODO(clalancette): This should be reenabled when Cyclone supports
      // reporting inconsistent topic as an event
      if (status_kind != DDS_INCONSISTENT_TOPIC_STATUS) {
        status_mask_map[dds_entity] |=
            get_status_kind_from_rmw(current_event->event_type);
      }
    }
  }
...
static rmw_ret_t handle_active_events(rmw_events_t *events) {
  if (events) {
    for (size_t i = 0; i < events->event_count; ++i) {
      ...
      if (!is_event_supported(current_event->event_type) ||
          !static_cast<bool>(status_mask & get_status_kind_from_rmw(
...
extern "C" rmw_ret_t rmw_wait(rmw_subscriptions_t *subs,
                              rmw_guard_conditions_t *gcs, rmw_services_t *srvs,
                              rmw_clients_t *cls, rmw_events_t *evs,
...

  {
    ...
    if (ws->inuse) {
      RMW_SET_ERROR_MSG(
          "concurrent calls to rmw_wait on a single waitset is not supported");
      return RMW_RET_ERROR;
    }
    ws->inuse = true;
  }
  ...
  if (require_reattach(ws->subs, subs ? subs->subscriber_count : 0,
      ...
      require_reattach(ws->srvs, srvs ? srvs->service_count : 0,
                       srvs ? srvs->services : nullptr) ||
      require_reattach(ws->cls, cls ? cls->client_count : 0,
                       cls ? cls->clients : nullptr) ||
...
using get_matched_endpoints_fn_t = dds_return_t (*)(dds_entity_t h,
                                                    dds_instance_handle_t *xs,
                                                    size_t nxs);
using BuiltinTopicEndpoint =
    std::unique_ptr<dds_builtintopic_endpoint_t,
                    std::function<void(dds_builtintopic_endpoint_t *)>>;

static rmw_ret_t
get_matched_endpoints(dds_entity_t h, get_matched_endpoints_fn_t fn,
                      std::vector<dds_instance_handle_t> &res) {
  dds_return_t ret;
  if ((ret = fn(h, res.data(), res.size())) < 0) {
    return RMW_RET_ERROR;
  }
  while (static_cast<size_t>(ret) >= res.size()) {
    // 128 is a completely arbitrary margin to reduce the risk of having to
    // retry when matches are create/deleted in parallel
...
static BuiltinTopicEndpoint
get_matched_subscription_data(dds_entity_t writer,
                              dds_instance_handle_t readerih) {
...
static BuiltinTopicEndpoint
get_matched_publication_data(dds_entity_t reader,
                             dds_instance_handle_t writerih) {
...
static rmw_ret_t rmw_take_response_request(CddsCS *cs,
  ...
  while (dds_take(cs->sub->enth, &wrap_ptr, &info, 1, 1) == 1) {
    if (info.valid_data) {
      static_assert(sizeof(request_header->request_id.writer_guid) ==
                        sizeof(wrap.header.guid) +
                            sizeof(info.publication_handle),
                    "request header size assumptions not met");
      memcpy(static_cast<void *>(request_header->request_id.writer_guid),
             static_cast<const void *>(&wrap.header.guid),
             sizeof(wrap.header.guid));
      memcpy(static_cast<void *>(request_header->request_id.writer_guid +
                                 sizeof(wrap.header.guid)),
             static_cast<const void *>(&info.publication_handle),
             sizeof(info.publication_handle));
      request_header->request_id.sequence_number = wrap.header.seq;
      request_header->source_timestamp = info.source_timestamp;
      // TODO(iluetkeb) get received_timestamp from Cyclone when implemented
      // there
...
extern "C" rmw_ret_t rmw_take_response(const rmw_client_t *client,
                                       rmw_service_info_t *request_header,
  ...
  if (ret == RMW_RET_OK && *taken) {
    ...
    if (dtreq > DDS_MSECS(REPORT_LATE_MESSAGES) ||
        dtresp > DDS_MSECS(REPORT_LATE_MESSAGES)) {
      fprintf(stderr, "** response time %.fms; response in history for %.fms\n",
              static_cast<double>(dtreq) / 1e6,
...
#endif
  TRACETOOLS_TRACEPOINT(
      rmw_take_response, static_cast<const void *>(client),
      static_cast<const void *>(ros_response),
      (nullptr != request_header ? request_header->request_id.sequence_number
                                 : 0LL),
...
extern "C" rmw_ret_t rmw_take_request(const rmw_service_t *service,
                                      rmw_service_info_t *request_header,
  ...
  rmw_ret_t ret = rmw_take_response_request(&info->service, request_header,
                                            ros_request, taken, nullptr, false);
  if (TRACETOOLS_TRACEPOINT_ENABLED(rmw_take_request)) {
    // Do not use the whole request_header->writer_guid, see the rmw_client_init
    // tracepoint trigger
    rmw_gid_t gid{};
    memcpy(gid.data, &request_header->request_id.writer_guid,
           sizeof(info->service.pub->pubiid));
    TRACETOOLS_DO_TRACEPOINT(
        rmw_take_request, static_cast<const void *>(service),
        static_cast<const void *>(ros_request), gid.data,
        (nullptr != request_header ? request_header->request_id.sequence_number
                                   : 0LL),
        *taken);
  }
...
static rmw_ret_t rmw_send_response_request(CddsCS *cs,
                                           const cdds_request_header_t &header,
                                           const void *ros_data,
                                           const dds_time_t timestamp) {
  const cdds_request_wrapper_t wrap = {header, const_cast<void *>(ros_data)};
  if (dds_write_ts(cs->pub->enth, static_cast<const void *>(&wrap),
                   timestamp) >= 0) {
    return RMW_RET_OK;
  } else {
...
static bool check_client_service_endpoint(const dds_builtintopic_endpoint_t *ep,
...
static client_present_t
check_for_response_reader(const CddsCS &service,
  ...
  if (reqwr == nullptr) {
    ...
    if (get_matched_endpoints(service.pub->enth, dds_get_matched_subscriptions,
                              rds) < 0) {
      RMW_SET_ERROR_MSG(
          "rmw_send_response: failed to get reader/writer matches");
...
extern "C" rmw_ret_t rmw_send_response(const rmw_service_t *service,
  ...
  static_assert(sizeof(request_header->writer_guid) ==
                    sizeof(header.guid) + sizeof(reqwrih),
                "request header size assumptions not met");
  memcpy(static_cast<void *>(&header.guid),
         static_cast<const void *>(request_header->writer_guid),
         sizeof(header.guid));
  memcpy(static_cast<void *>(&reqwrih),
         static_cast<const void *>(request_header->writer_guid +
                                   sizeof(header.guid)),
         sizeof(reqwrih));
  header.seq = request_header->sequence_number;
  // Block until the response reader has been matched by the response writer
  // (this is a workaround: rmw_service_server_is_available should keep
  // returning false until this is a given).
  ...
  while ((st = check_for_response_reader(info->service, reqwrih)) ==
  ...
  case client_present_t::YES: {
    ...
    if (TRACETOOLS_TRACEPOINT_ENABLED(rmw_send_response)) {
      // Do not use request_header->writer_guid, see the rmw_client_init
      // tracepoint trigger
      rmw_gid_t gid{};
      memcpy(gid.data, &header.guid, sizeof(header.guid));
      TRACETOOLS_DO_TRACEPOINT(rmw_send_response,
                               static_cast<const void *>(service),
                               static_cast<const void *>(ros_response),
                               gid.data, header.seq, timestamp);
    }
...
extern "C" rmw_ret_t rmw_send_request(const rmw_client_t *client,
  ...
  TRACETOOLS_TRACEPOINT(rmw_send_request, static_cast<const void *>(client),
                        static_cast<const void *>(ros_request), header.seq);
  return rmw_send_response_request(&info->client, header, ros_request,
                                   timestamp);
}

static const rosidl_service_type_support_t *
get_service_typesupport(const rosidl_service_type_support_t *type_supports) {
...
static void get_unique_csid(const rmw_node_t *node, client_service_id_t &id) {
  ...
  static_assert(sizeof(dds_guid_t) <= RMW_GID_STORAGE_SIZE,
...
static rmw_ret_t rmw_init_cs(CddsCS *cs, user_callback_data_t *cb_data,
  ...
  const rosidl_service_type_support_t *type_support =
      get_service_typesupport(type_supports);
  ...
  if (is_service) {
    ...
    subtopic_name = make_fqtopic(ROS_SERVICE_REQUESTER_PREFIX, service_name,
                                 "Request", qos_policies);
    pubtopic_name = make_fqtopic(ROS_SERVICE_RESPONSE_PREFIX, service_name,
                                 "Reply", qos_policies);
  } else {
    ...
    sub_type_hash = type_supports->response_typesupport->get_type_hash_func(
        type_supports->response_typesupport);
    pubtopic_name = make_fqtopic(ROS_SERVICE_REQUESTER_PREFIX, service_name,
                                 "Request", qos_policies);
    subtopic_name = make_fqtopic(ROS_SERVICE_RESPONSE_PREFIX, service_name,
                                 "Reply", qos_policies);
  }
  ...
  RCUTILS_LOG_DEBUG_NAMED("rmw_cyclonedds_cpp", "Sub Topic %s",
                          subtopic_name.c_str());
  RCUTILS_LOG_DEBUG_NAMED("rmw_cyclonedds_cpp", "Pub Topic %s",
                          pubtopic_name.c_str());
  RCUTILS_LOG_DEBUG_NAMED("rmw_cyclonedds_cpp", "***********");
  ...
  pub_st = create_sertype(type_support->typesupport_identifier,
                          pub_type_support, true, std::move(pub_msg_ts));
  struct ddsi_sertype *pub_stact;
  pubtopic = create_topic(node->context->impl->ppant, pubtopic_name.c_str(),
                          pub_st, &pub_stact);
  if (pubtopic < 0) {
  ...
  sub_st = create_sertype(type_support->typesupport_identifier,
                          sub_type_support, true, std::move(sub_msg_ts));
  subtopic =
      create_topic(node->context->impl->ppant, subtopic_name.c_str(), sub_st);
  if (subtopic < 0) {
  ...
  user_data = std::string(is_service ? "serviceid=" : "clientid=") +
              csid_to_string(cs->id) + std::string(";");
  ...
  if (RMW_RET_OK != rmw_dds_common::encode_sertype_hash_for_user_data_qos(
    ...
    RCUTILS_LOG_WARN_NAMED("rmw_cyclonedds_cpp",
                           "Failed to encode service type hash for service, "
                           "will not distribute it in USER_DATA.");
  ...
  if ((pub_qos = create_readwrite_qos(qos_policies, *pub_type_hash, false,
                                      user_data)) == nullptr) {
    goto fail_pub_qos;
  }
  if ((sub_qos = create_readwrite_qos(qos_policies, *sub_type_hash, false,
                                      user_data)) == nullptr) {
    goto fail_sub_qos;
  }

  if ((pub->enth = dds_create_writer(node->context->impl->dds_pub, pubtopic,
                                     pub_qos, nullptr)) < 0) {
    RMW_SET_ERROR_MSG("failed to create writer");
    goto fail_writer;
  }
  ...
  if ((sub->enth = dds_create_reader(node->context->impl->dds_sub, subtopic,
                                     sub_qos, listener)) < 0) {
    RMW_SET_ERROR_MSG("failed to create reader");
    goto fail_reader;
  }
...
extern "C" rmw_client_t *rmw_create_client(
  ...
  auto cleanup_info = rcpputils::make_scope_exit([info]() { delete (info); });
  ...
  rmw_qos_profile_t adapted_qos_policies =
      rmw_dds_common::qos_profile_update_best_available_for_services(
          *qos_policies);
  if (rmw_init_cs(&info->client, &info->user_callback_data, node, type_supports,
                  service_name, &adapted_qos_policies, false) != RMW_RET_OK) {
    return nullptr;
  }
  auto cleanup_fini_cs =
      rcpputils::make_scope_exit([info]() { rmw_fini_cs(&info->client); });
  ...
  auto cleanup_client = rcpputils::make_scope_exit(
      [rmw_client]() { rmw_client_free(rmw_client); });
  ...
  auto cleanup_service_name = rcpputils::make_scope_exit([rmw_client]() {
    rmw_free(const_cast<char *>(rmw_client->service_name));
  });
  memcpy(const_cast<char *>(rmw_client->service_name), service_name,
         strlen(service_name) + 1);

  // Update graph
  if (RMW_RET_OK != common->add_client_graph(info->client.pub->gid,
                                             info->client.sub->gid, node->name,
                                             node->namespace_)) {
    return nullptr;
  }
  ...
  if (TRACETOOLS_TRACEPOINT_ENABLED(rmw_client_init)) {
    // rmw_cyclonedds uses info->client.pub->pubiid as the internal request
    // header GUID, which is the first half (8 bytes out of 16 bytes) of the
    // rmw_request_id_t's writer_guid. The second half doesn't match when read
    // from the client side and the service side, so only use the first half.
    // The second half will be zeros on both client side and service side.
    rmw_gid_t gid{};
    memcpy(gid.data, &info->client.pub->pubiid,
           sizeof(info->client.pub->pubiid));
    TRACETOOLS_DO_TRACEPOINT(rmw_client_init,
                             static_cast<const void *>(rmw_client), gid.data);
  }
  return rmw_client;
}

extern "C" rmw_ret_t rmw_destroy_client(rmw_node_t *node,
                                        rmw_client_t *client) {
  RMW_CHECK_ARGUMENT_FOR_NULL(node, RMW_RET_INVALID_ARGUMENT);
  RMW_CHECK_TYPE_IDENTIFIERS_MATCH(node, node->implementation_identifier,
                                   eclipse_cyclonedds_identifier,
  ...
  // Update graph
  auto common = &node->context->impl->common;
  if (RMW_RET_OK != common->remove_client_graph(info->client.pub->gid,
                                                info->client.sub->gid,
                                                node->name, node->namespace_)) {
    RMW_SET_ERROR_MSG(
        "failed to publish ParticipantEntitiesInfo when destroying client");
  }
...
extern "C" rmw_service_t *rmw_create_service(
  ...
  rmw_qos_profile_t adapted_qos_policies =
      rmw_dds_common::qos_profile_update_best_available_for_services(
          *qos_policies);
  if (rmw_init_cs(&info->service, &info->user_callback_data, node,
                  type_supports, service_name, &adapted_qos_policies,
                  true) != RMW_RET_OK) {
    return nullptr;
  }
  auto cleanup_fini_cs =
      rcpputils::make_scope_exit([info]() { rmw_fini_cs(&info->service); });
  rmw_service_t *rmw_service = rmw_service_allocate();
  if (!rmw_service) {
    return nullptr;
  }
  auto cleanup_service = rcpputils::make_scope_exit(
      [rmw_service]() { rmw_service_free(rmw_service); });
  auto common = &node->context->impl->common;
  ...
  auto cleanup_service_name = rcpputils::make_scope_exit([rmw_service]() {
  ...
  // Update graph
  if (RMW_RET_OK != common->add_service_graph(info->service.sub->gid,
...
extern "C" rmw_ret_t rmw_destroy_service(rmw_node_t *node,
  ...
  if (RMW_RET_OK != common->remove_service_graph(
                        info->service.sub->gid, info->service.pub->gid,
                        node->name, node->namespace_)) {
    RMW_SET_ERROR_MSG(
        "failed to publish ParticipantEntitiesInfo when destroying service");
  }
...
extern "C" rmw_ret_t
rmw_get_node_names(const rmw_node_t *node, rcutils_string_array_t *node_names,
  ...
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  return common_context->graph_cache.get_node_names(node_names, node_namespaces,
                                                    nullptr, &allocator);
}
...
extern "C" rmw_ret_t
rmw_get_service_names_and_types(const rmw_node_t *node,
                                rcutils_allocator_t *allocator,
...
static rmw_ret_t get_topic_name(dds_entity_t endpoint_handle,
                                std::string &name) {
  std::vector<char> tmp(128);
  dds_return_t rc =
      dds_get_name(dds_get_topic(endpoint_handle), tmp.data(), tmp.size());
...
static rmw_ret_t check_for_service_reader_writer(const CddsCS &client,
                                                 bool *is_available) {
  std::vector<dds_instance_handle_t> rds, wrs;
  assert(is_available != nullptr && !*is_available);
  if (get_matched_endpoints(client.pub->enth, dds_get_matched_subscriptions,
                            rds) < 0 ||
      get_matched_endpoints(client.sub->enth, dds_get_matched_publications,
                            wrs) < 0) {
    RMW_SET_ERROR_MSG(
        "rmw_service_server_is_available: failed to get reader/writer matches");
    return RMW_RET_ERROR;
  }
...
extern "C" rmw_ret_t rmw_service_server_is_available(const rmw_node_t *node,
  ...
  *is_available = false;
  ...
  if (get_topic_name(info->client.pub->enth, pub_topic_name) < 0 ||
    ...
    RMW_SET_ERROR_MSG(
        "rmw_service_server_is_available: failed to get topic names");
    return RMW_RET_ERROR;
  }

  size_t number_of_request_subscribers = 0;
  rmw_ret_t ret = common_context->graph_cache.get_reader_count(
      pub_topic_name, &number_of_request_subscribers);
  ...
  ret = common_context->graph_cache.get_writer_count(
      sub_topic_name, &number_of_response_publishers);
...
extern "C" rmw_ret_t rmw_count_publishers(const rmw_node_t *node,
                                          const char *topic_name,
                                          size_t *count) {
  RMW_CHECK_ARGUMENT_FOR_NULL(node, RMW_RET_INVALID_ARGUMENT);
  RMW_CHECK_TYPE_IDENTIFIERS_MATCH(node, node->implementation_identifier,
  ...
  const std::string mangled_topic_name =
      make_fqtopic(ROS_TOPIC_PREFIX, topic_name, "", false);
  return common_context->graph_cache.get_writer_count(mangled_topic_name,
                                                      count);
}

extern "C" rmw_ret_t rmw_count_subscribers(const rmw_node_t *node,
                                           const char *topic_name,
                                           size_t *count) {
  RMW_CHECK_ARGUMENT_FOR_NULL(node, RMW_RET_INVALID_ARGUMENT);
  RMW_CHECK_TYPE_IDENTIFIERS_MATCH(node, node->implementation_identifier,
  ...
  const std::string mangled_topic_name =
      make_fqtopic(ROS_TOPIC_PREFIX, topic_name, "", false);
  return common_context->graph_cache.get_reader_count(mangled_topic_name,
                                                      count);
}

extern "C" rmw_ret_t rmw_count_clients(const rmw_node_t *node,
                                       const char *service_name,
                                       size_t *count) {
  RMW_CHECK_ARGUMENT_FOR_NULL(node, RMW_RET_INVALID_ARGUMENT);
  RMW_CHECK_TYPE_IDENTIFIERS_MATCH(node, node->implementation_identifier,
  ...
  return common_context->graph_cache.get_reader_count(mangled_rp_service_name,
                                                      count);
}

extern "C" rmw_ret_t rmw_count_services(const rmw_node_t *node,
                                        const char *service_name,
                                        size_t *count) {
  RMW_CHECK_ARGUMENT_FOR_NULL(node, RMW_RET_INVALID_ARGUMENT);
  RMW_CHECK_TYPE_IDENTIFIERS_MATCH(node, node->implementation_identifier,
  ...
  RMW_CHECK_ARGUMENT_FOR_NULL(count, RMW_RET_INVALID_ARGUMENT);
  auto common_context = &node->context->impl->common;
  const std::string mangled_rq_topic_name = make_fqtopic(
      ROS_SERVICE_REQUESTER_PREFIX, service_name, "Request", false);
  return common_context->graph_cache.get_reader_count(mangled_rq_topic_name,
                                                      count);
}

using GetNamesAndTypesByNodeFunction =
    rmw_ret_t (*)(rmw_dds_common::Context *, const std::string &,
                  const std::string &, DemangleFunction, DemangleFunction,
                  rcutils_allocator_t *, rmw_names_and_types_t *);
...
static rmw_ret_t get_topic_names_and_types_by_node(
    ...
    DemangleFunction demangle_topic, DemangleFunction demangle_type,
    bool no_demangle,
  ...
  if (RMW_NODE_NAME_VALID != validation_result) {
    const char *reason =
        rmw_node_name_validation_result_string(validation_result);
    RMW_SET_ERROR_MSG_WITH_FORMAT_STRING("node_name argument is invalid: %s",
                                         reason);
    return RMW_RET_INVALID_ARGUMENT;
  }
  validation_result = RMW_NAMESPACE_VALID;
  ...
  return get_names_and_types_by_node(common_context, node_name, node_namespace,
                                     demangle_topic, demangle_type, allocator,
                                     topic_names_and_types);
}

static rmw_ret_t get_reader_names_and_types_by_node(
    rmw_dds_common::Context *common_context, const std::string &node_name,
    const std::string &node_namespace, DemangleFunction demangle_topic,
    DemangleFunction demangle_type, rcutils_allocator_t *allocator,
    rmw_names_and_types_t *topic_names_and_types) {
  return common_context->graph_cache.get_reader_names_and_types_by_node(
      node_name, node_namespace, demangle_topic, demangle_type, allocator,
      topic_names_and_types);
}
...
extern "C" rmw_ret_t rmw_get_subscriber_names_and_types_by_node(
  ...
  return get_topic_names_and_types_by_node(
      ...
      _demangle_ros_topic_from_topic, _demangle_if_ros_type, no_demangle,
      get_reader_names_and_types_by_node, tptyp);
}
...
extern "C" rmw_ret_t rmw_get_publisher_names_and_types_by_node(
  ...
  return get_topic_names_and_types_by_node(
      node, allocator, node_name, node_namespace,
      _demangle_ros_topic_from_topic, _demangle_if_ros_type, no_demangle,
      get_writer_names_and_types_by_node, tptyp);
}
...
extern "C" rmw_ret_t rmw_get_service_names_and_types_by_node(
  ...
  return get_topic_names_and_types_by_node(
      node, allocator, node_name, node_namespace,
      _demangle_service_request_from_topic, _demangle_service_type_only, false,
      get_reader_names_and_types_by_node, sntyp);
}
...
extern "C" rmw_ret_t rmw_get_client_names_and_types_by_node(
  ...
  return get_topic_names_and_types_by_node(
      node, allocator, node_name, node_namespace,
      _demangle_service_reply_from_topic, _demangle_service_type_only, false,
      get_reader_names_and_types_by_node, sntyp);
}
...
extern "C" rmw_ret_t rmw_get_subscriptions_info_by_topic(
  ...
  RMW_CHECK_ARGUMENT_FOR_NULL(topic_name, RMW_RET_INVALID_ARGUMENT);
  if (RMW_RET_OK !=
      rmw_topic_endpoint_info_array_check_zero(subscriptions_info)) {
    return RMW_RET_INVALID_ARGUMENT;
  }
...
extern "C" rmw_ret_t rmw_get_clients_info_by_service(
  ...
  auto common_context = &node->context->impl->common;
  std::string mangled_rq_topic_name = make_fqtopic(
      ROS_SERVICE_REQUESTER_PREFIX, service_name, "Request", false);
